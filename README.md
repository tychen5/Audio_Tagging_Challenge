...
- 使用MFCC於google drive feature_all/mfcc中的train_X來訓練model (原本phase1的hyper parameter不要動，直接用train_X shuffle切6-fold來重train六個model)
- 全部label放在feature_all資料夾中
- train完以後一個model要predict兩次，所以一個人總共12次 (要predict train_X，跟predict test_X)
- 選val_loss最低的兩個model predict出來的test_X；只選softmax結果最高的類別(僅predict一個label)，丟上去kaggle，於google drive表單填上public score
