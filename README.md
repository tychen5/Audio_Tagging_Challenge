...
Project Manager: tychen5(leo)

Report: https://mega.nz/#!tgUiEI5A!j4rDzx-uVty-ZhG41g7DeVxqsMMy6PoTI0-FLtH0TSI

Presentation: https://docs.google.com/presentation/d/1yTzUhxhkXHE6ZztCp7nreX9FXbE5nB3F95Z-vzQI_2k/edit?fbclid=IwAR0viQ13dV3MKEq6-GKUoGXdDzAaTcpl8kZ1YELkqOVzSXyRT3Qy-oUIljs#slide=id.p

Problem Description: https://docs.google.com/presentation/d/1aelUEAI3fMBR4gbmxFtG227u1dT-zVRn1CFWoQz26m0/edit#slide=id.p3

***
* 太大的檔案放置於雲端: https://drive.google.com/drive/folders/1LdcR5EPBbYdcC8ltlp83B-deS78xOYOL?usp=sharing
* 經整理的code與report放置於final繳交資料夾


#### Phase 3 self-train ####
- 每個model的每個fold上傳unverified+testing(一萬五千多筆)的fname,softmax CSV到雲端: https://drive.google.com/open?id=1PNazIuFvk26HihaFqBVyVjz1jm6oa4f_
...
- 雲端含有ensemble verified過後的fname,label csv進行self-train fine tune ，如果某fold持平或進步的話就更改acc表單並改用新的model
- 先做一次各fold predict全部9400 testing上傳到phase3，所以一個model會有10個csv (fname,softmax): https://drive.google.com/drive/u/3/folders/1QvdPf_atYdv7ge9xKqIqoZBrQGRds_x- (直接點會error就用複製的ㄅ)，然後可改一些參數繼續fine tune到隔天(co-train或再自己self-train皆可)
- 每個model的每個fold要predict該fold的validation data跟全部的testing data，把10 fold 的validation data append在一起變成完整的3710筆fname,sotmax CSV。所以每種model會predict出11個csv ( https://drive.google.com/drive/u/3/folders/17yjI9OeAxZofIOi611rWhjrumgXv2UyF  )
...
- 上傳該NN去predict全部testing data的結果(fname,probability distribution)，並填stacking_acc表單: https://drive.google.com/drive/u/3/folders/1zDNHnUDjAodJLhkU6XBserlDP7nbzI8u
...
- 如果自己有兩個以上model的話，就隨便再多挑其他人的來做semi

====================以上第一次co-train ===============

- 討論semi supervised learning、Phase3、Phase4、戳public data set
...
- 討論遇到的問題，說明phase2 stage2,3，phase3
...
- 上傳好10-fold CV predict結果的csv(20個檔案)到google drive: feature_all/predict_valid_data & feature_all/predict_unverified_data
  
...
- 大家可以任意上傳到kaggle去直接測試自己的model acc

- 討論遇到的問題、IDEA、創意、架構、方法改良~~
